...
func init() {
	// Initialize the CLI app and start FGeth
	app.Action = fgeth
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2021 The FG-ethereum Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.DevnetFlag.Name):
		log.Info("Starting FGeth on Devnet testnet...")
	...
	if ctx.GlobalString(utils.SyncModeFlag.Name) != "light" && !ctx.GlobalIsSet(utils.CacheFlag.Name) && !ctx.GlobalIsSet(utils.NetworkIdFlag.Name) {
		...
		if !ctx.GlobalIsSet(utils.DevnetFlag.Name) && !ctx.GlobalIsSet(utils.DeveloperFlag.Name) {
